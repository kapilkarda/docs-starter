...
## Vobiz SIP Integration Guide — Vocbot SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Vobiz SIP trunk** to Vocbot so their
...
> number*. Vocbot gives you back a *SIP URI* that Vobiz must forward inbound calls to. That
...
```
   ...
   Vocbot → Vobiz SIP domain (termination) ────► sends INVITE to
...
- **Outbound = Vocbot → Vobiz.** From Vocbot's point of view, Vobiz is where the call *terminates*
  into the phone network. That is why Vocbot calls it the **SIP Termination URI**.
- **Inbound = Vobiz → Vocbot.** Vobiz must be told to *originate* the incoming call toward Vocbot's
  SIP address.

---

## Credential cheat-sheet (Vobiz → Vocbot field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the Vocbot dashboard
in Part B.

| What you get from Vobiz            | Example value                     | Vocbot dashboard field    |
| ---------------------------------- | --------------------------------- | ------------------------ |
| Trunk **SIP domain**               | `abc123.sip.vobiz.ai`             | **SIP Termination URI**  |
| Trunk **credential username**      | `vocbot_prod`                      | **SIP Username**         |
...
Then, after saving in Vocbot, you receive **one** value to bring *back* to Vobiz:

| What Vocbot gives you           | Example value                       | Where it goes in Vobiz            |
| ------------------------------ | ----------------------------------- | --------------------------------- |
...
2. After login, open the dashboard. Note your account-level **Auth ID** and **Auth Token** — these
   authenticate any Vobiz API calls. (You don't paste these into Vocbot; they're only for managing
   Vobiz itself.)
...
2. Give it a name (e.g. `Vocbot-trunk`) and set the trunk type to support both directions
...
2. Set a **username** (e.g. `vocbot_prod`) and a **strong password** (12+ chars, mixed case, digits,
   symbols).
...
> Vobiz also supports IP-based auth. Vocbot authenticates with **username/password**, so credential
...
2. Leave the inbound destination for now — you'll point it at Vocbot in **Part C**, after Vocbot
...
4. Fill in the form using your cheat-sheet:
   ...
   | **SIP Username**     | The Vobiz credential username, e.g. `vocbot_prod`          |
...
2. Set the inbound **destination / origination URI** to your Vocbot **SIP URI** from Part B.
...
<Frame>
    <img src="/images/vobiz/06-inbound-origination.png" alt="Vobiz console: inbound destination / origination URI set to the Vocbot SIP URI" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** Vocbot → `abc123.sip.vobiz.ai` (your Vobiz trunk) → PSTN.
- **Inbound:** Caller → your DID → Vobiz → `178.212.35.170:5060` (Vocbot) → AI agent.
...
| Outbound rejected (number/format)                   | DID must be E.164 with `+` in Vocbot. Confirm the DID is enabled for outbound on Vobiz.                  |
...
| Password stopped working                            | Vobiz passwords are write-once. If rotated/lost, create a new Vobiz credential and update the Vocbot dialer. |
