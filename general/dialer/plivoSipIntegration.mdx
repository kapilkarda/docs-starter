...
## Plivo SIP Integration Guide — Vocbot SIP Dialer (Inbound + Outbound)

This guide walks a customer through connecting a **Plivo SIP trunk** to Vocbot so their
...
- **Part A — Plivo side:** buy a number, create an Outbound Trunk, generate SIP credentials, and get the
  values Vocbot needs.
- **Part B — Vocbot side:** add the SIP Dialer in the Vocbot dashboard, then finish the inbound
  loop-back on Plivo.

> **You need one thing from each side.** Plivo gives you a *SIP termination domain + username + password +
> phone number*. Vocbot gives you back a *SIP URI* that Plivo must forward inbound calls to. That
...
```
   ...
   Vocbot → Plivo termination URI ─────────────► sends INVITE to
   ...
   your Vocbot SIP URI  ────────────────► Inbound Trunk (matches the number)
                                                 ──► Dispatch Rule ──► AI agent answers
```

- **Outbound = Vocbot → Plivo.** From Vocbot's point of view, Plivo is where the call *terminates*
  into the phone network. That is why Vocbot calls it the **SIP Termination URI**.
- **Inbound = Plivo → Vocbot.** Plivo must be told to *originate* the incoming call toward Vocbot's
  SIP address (set as the Plivo **Inbound Trunk Primary URI**).

---

## Credential cheat-sheet (Plivo → Vocbot field mapping)

Fill this in as you go through Part A. You'll paste the right-hand column into the Vocbot dashboard
in Part B.

| What you get from Plivo                     | Example value                     | Vocbot dashboard field    |
| ------------------------------------------- | --------------------------------- | ------------------------ |
| Outbound Trunk **termination SIP domain**   | `21784177241578.zt.plivo.com`     | **SIP Termination URI**  |
| Outbound Trunk **credential username**      | `vocbotprod`                       | **SIP Username**         |
...
Then, after saving in Vocbot, you receive **one** value to bring *back* to Plivo:

| What Vocbot gives you           | Example value                       | Where it goes in Plivo                  |
| ------------------------------ | ----------------------------------- | --------------------------------------- |
| Your **SIP URI**       | `myproj-x1y2.sip.livekit.cloud`     | Inbound Trunk **Primary URI**           |

> The exact SIP URI is shown on your dialer's **SIP configuration** in the Vocbot dashboard
...
2. After login, open the dashboard. Plivo's SIP trunking product is called **Zentrunk** — you'll find
   ...
   (these authenticate Plivo API calls; you do **not** paste them into Vocbot).
...
4. Record it in **E.164 format with the `+` and country code** — e.g. `+12025550147`.
   → This becomes the **Phone Number** field in Vocbot.
...
The Outbound Trunk is what Vocbot sends outbound calls **into** — it is the source of your
**SIP Termination URI**.

1. Go to **SIP Trunking → Outbound Trunks** and click **Create New Outbound Trunk**.
2. Under **Trunk Details**, enter a name (e.g. `Vocbot-outbound`).
3. For **Trunk Authentication**, choose **Credentials List** (Vocbot authenticates with
...
5. On save, Plivo provisions a unique **termination SIP domain** in the form
   ...
   → This becomes the **SIP Termination URI** in Vocbot.
...
2. Enter an **Auth Group** name (e.g. `Vocbot-creds`), a **username**, and a **password**.
   - Username: **5–20 alphanumeric** characters (e.g. `vocbotprod`).
...
in Step 2. (You'll set its **Primary URI** later in Part C, once Vocbot gives you the SIP URI.)

1. Go to **SIP Trunking → Inbound Trunks** and click **Create New Inbound Trunk**.
2. Enter a name (e.g. `Vocbot-inbound`).
...
# Part B — Vocbot dashboard: add the SIP Dialer

This is what a customer actually clicks in the Vocbot app.

1. Log in to the **Vocbot dashboard**.
...
4. Fill in the form using your cheat-sheet:
   ...
   | **SIP Username**     | The Plivo credential username, e.g. `vocbotprod`            |
   | **SIP Password**     | The Plivo credential password (paste the one you copied)   |

5. Click **Create / Save**.

**What Vocbot does behind the scenes** (no action needed from you):
...
**What Vocbot gives you back:** on success, the dialer now exists and Vocbot exposes your
**SIP URI** (shown in the dialer's SIP config / setup instructions, format
`178.212.35.170:5060`). **Copy it — you need it for Part C.**

> If you don't see the SIP URI in the UI, it's a fixed value for your Vocbot workspace — ask Vocbot
...
1. Return to the **Plivo console** → **SIP Trunking → Inbound Trunks** → open the inbound trunk from
   Step 5.
2. Set the **Primary URI** to your Vocbot **SIP URI** from Part B:
...
<Frame>
    <img src="/images/plivo/08-primary-uri.jpeg" alt="Plivo console: Inbound Trunk Primary URI set to the Vocbot SIP URI with ;transport=tcp" style={{ borderRadius: '0.5rem' }} />
</Frame>

Now the loop is closed:

- **Outbound:** Vocbot → `21784177241578.zt.plivo.com` (your Plivo trunk) → PSTN.
- **Inbound:** Caller → your DID → Plivo → `178.212.35.170:5060` (Vocbot) → AI agent.
...
**Outbound test**
1. In Vocbot, assign the new SIP dialer to a campaign or a Playground test call.
...
2. It should connect to your AI agent (assign an inbound assistant to this dialer/number in Vocbot
   first).
...
| Outbound rejected (number/format)                   | DID must be E.164 with `+` in Vocbot. Confirm the DID is Voice-capable on Plivo.                        |
...
| "This phone number is already being used"           | The DID is already registered to another Vocbot dialer/workspace. Delete the old dialer or use a new DID. |
...
| Password stopped working                            | Plivo passwords are write-once. If rotated/lost, create a new credential and update the Vocbot dialer.  |
...
- [Vocbot — Vobiz SIP Integration](https://vocbot.mintlify.app//general/dialer/vobizSipIntegration) (sister guide)
