...
description: "This document explains how to configure and receive webhooks from the **vocbot** platform. A webhook lets your vocbot assistant forward collected call information to your own server or any 3rd-party service (CRM, Zapier, Make, your API, Google Sheets, etc.) in real time."
---


## 1. Overview

vocbot supports **two webhook trigger modes**. You configure them from the **Tools** section of your assistant inside the vocbot Dashboard.
...
2. vocbot takes the full transcript + collected context.
...
4. vocbot sends an HTTP request (method of your choice) to your configured URL with headers, query parameters, and body.
5. Your server responds. The response status + body is logged in the vocbot delivery history for audit.

---

## 3. Configuring the Webhook in the vocbot Dashboard
...
| **Body Parameters** | The fields you want vocbot to send in the request body. Each parameter has: `Identifier`, `Data Type`, `Required` flag, `Description`, `Value`. |
...
| **Value** | Leave blank. vocbot will automatically fill this from the call transcript when the call ends. |
...
**Webhook URL example:** `https://your-domain.com/api/vocbot/call-report`
**Method:** `POST`
...
When the call ends, vocbot sends a request like this to the URL you configured:

**Request**
```
POST /api/vocbot/call-report HTTP/1.1
...
```json
{
  ...
  "recording_url": "https://cdn.vocbot.ai/recordings/cs_abc123.mp3"
}
```
...
Your server should respond with **HTTP 200 or 201** and a JSON body. Non-2xx responses are logged as failures in vocbot's delivery history.
...
vocbot automatically serializes the body based on the `Content-Type` header you set:
...
2. **Use the webhook.site URL** as the Webhook URL in the vocbot Dashboard while testing:
   ```
   https://webhook.site/<your-unique-id>
   ```
3. **Place a test call** through the vocbot dashboard or playground.
...
app.post("/api/vocbot/call-report", (req, res) => {
  console.log("vocbot webhook received:", req.body);
...
Contact your vocbot account manager for access to the delivery log for your workspace.
...
For issues, error codes, or custom integrations, reach out to your vocbot support contact with:
- Assistant name
- Approximate call timestamp (with timezone)
- Your server's response code + body

We will match it against vocbot's delivery log for debugging.
